#!/usr/bin/env bash
# 一键部署本地 Worker（KV 需先创建，见 README「部署·方式 D」）：
#   1. 可选：用 KV_ID 覆盖 wrangler.toml 中的 KV 命名空间 id
#   2. 可选：推送 MANIFEST 到 KV
...
#   可选  KV_ID             已有 KV 命名空间的 id（若 wrangler.toml 已填好则无需设置）
#   可选  MANIFEST          路由表 JSON（写入 KV key "manifest"，覆盖内置默认）
#   可选  CF_DOMAIN         自定义域名（须已托管在该 Cloudflare 账号）
#
# 注意：本脚本不会自动创建 KV 命名空间——请用
#   wrangler kv namespace create KV
# 把返回的 id 填进 wrangler.toml 的 kv_namespaces（或通过 KV_ID 注入）。
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ -n "${KV_ID:-}" ]; then
  echo "[deploy] KV -> id $KV_ID"
  sed -i -E 's/(binding = "KV",[[:space:]]*id = ")[^"]*/\1'"$KV_ID"'/' wrangler.toml
fi

if [ -n "${MANIFEST:-}" ]; then
  ID="$(sed -n 's/.*binding = "KV", id = "\([^"]*\)".*/\1/p' wrangler.toml | head -n1)"
  if [ -n "$ID" ] && [ "$ID" != "REPLACE_WITH_KV_ID" ]; then
    echo "[deploy] writing MANIFEST to KV"
    printf '%s' "$MANIFEST" | wrangler kv key put manifest --namespace-id "$ID" --path -
  else
    echo "[deploy] skip MANIFEST: KV id 未配置"
  fi
fi
